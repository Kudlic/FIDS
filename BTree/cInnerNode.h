...
bool cInnerNode<T>::needAdjustment(int index) {
    ...
    if(this->getChild(index)->isLeafNode()){
        ...
        cTuple<T> tupleLowNode = cTuple<T> ((T*)this->getTupleLowPtr(index), this->metadata->n1, true);
        cTuple<T> tupleHighNode = cTuple<T> ((T*)this->getTupleHighPtr(index), this->metadata->n1, true);
