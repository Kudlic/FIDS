...
void SelectHashIndex(int* atrs_size, int len, int* attr_max_value, int atr_count, cRowHeapTable* table) {
    ...
    while (!out.eof() && i < len)
    {
        out.getline(str, MAX_LEN);
        char* rec = new char[len];
        cRowHeapTable::CreateHashRecord(rec, atrs_size, attr_max_value, atr_count, str);

        result = table->SelectHashIndex(rec);
        in << result << "\n";
        i++;
    }
    auto t2 = high_resolution_clock::now();
    in.close();
    duration<double> time_span = duration_cast<duration<double>>(t2 - t1);
    printf("Select with index:\nTime: %.2fs, Throughput: %.2f op/s.\n\n", time_span.count(), GetThroughput(len, time_span.count(), 1));
