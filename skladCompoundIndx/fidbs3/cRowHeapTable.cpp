...
string cRowHeapTable::GetRecord(int position)
{
	string result;
	char* pom = new char[mRowSize];
	memcpy(pom, &mData[mRowSize * position], mRowSize);
	result.assign(pom, mRowSize);
	int pos = 0;
	for (int i = 0; i < mAtrCount-1; i++) {
		pos += mAtrsSize[i];
		result.insert(pos, 1, ';');
		pos++;
	}
...
int cRowHeapTable::SelectHashIndex(int* ids, int* rec)
{
	...
	return hash->Select(ids, key);
}

int cRowHeapTable::SelectHashIndexStatistics(int* rec)
{
	...
	return hash->SelectStatistics(key);
}
