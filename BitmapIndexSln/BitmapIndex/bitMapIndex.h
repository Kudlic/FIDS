...
public:
    ...
    int Select(uint32_t **attrs, uint32_t attrsCount, int* resultRowIds = nullptr) {
        char * search = new char[this->mByteSize];
        char * mask = new char[this->mByteSize];
        memset(search, 0, this->mByteSize);
        memset(mask, 0, this->mByteSize);
