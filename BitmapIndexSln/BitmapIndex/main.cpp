...
void RowHeapTableTest(const int rowCount)
{
    ...
    department BYTE
    */
    uint32_t * schema = new uint32_t[5] {10,10,1,1,1};
    ...
    uint32_t** query = new uint32_t * [2];
    query[0] = new uint32_t[2]{ 2, 0 };
    query[1] = new uint32_t[2]{ 3, 1 };
...
void RowHeapTableBenchmark(const int rowCount)
{
    ...
    */
    ...
    int queryCnt = 1000;
    uint32_t*** query = new uint32_t * *[queryCnt];
    for (int i = 0; i < queryCnt; i++) {
        query[i] = new uint32_t * [3];
        for (int j = 0; j < 3; j++) {
            query[i][j] = new uint32_t[2];
        }
    }
    for(int i = 0; i < queryCnt; i++){
        query[i][0][0] = 2;
